...
{
    public class cRainEffect : cEffectBase
    {
        private const int FRAMES_PER_STEP = 4;
        private const int MAX_STEP = 10;

        private const int IMG_WIDTH = 41;
        private const int IMG_HEIGHT = 45;

        private static Image[] arrAnimation;

        public cRainEffect(Point oLocation)
        {
            try
            {
                ...
                if (arrAnimation == null)
                {
                    ...
                    for (int i = 0; i < MAX_STEP; i++)
                        arrAnimation[i] = Image.FromFile(sPath + @"Rain\" + (i).ToString().PadLeft(3, '0') + ".png");
                }
            }
